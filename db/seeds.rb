...
user = User.find_or_create_by!(email: 'alan@alanharper.com.au') do |user|
...
competitions.each do |name, closing_date_string|
  ...
  Entry.find_or_create_by!(competition: competition, user: user, section: open_section) do |entry|
  ...
  Entry.find_or_create_by!(competition: competition, user: user, section: theme_section) do |entry|
    entry.photo = ActionDispatch::Http::UploadedFile.new(
        tempfile: File.new(Rails.root.join('spec/fixtures/red_umbrella.jpg')),
        filename: File.basename(File.new(Rails.root.join('spec/fixtures/red_umbrella.jpg')))
    )
